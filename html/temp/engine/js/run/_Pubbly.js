...
class Pubbly {
    ...
    checkLocFor(loc, what, condition) {
        ...
        for (let l = 0; l < curPage.links.length; l++) {
            ...
            let poly = false;
            if ((link.type === "graphic" || link.type === "link") && link.enabled) {
                poly = link.poly;
    ...
    loadBufferLead(newPage, cbs) {
        ...
        if (missingForDraw === 0) {
            ...
            let progFnc = this.progressGraph.calc;
            if (!this.firstDraw) {
                this.slowNetworkGif.show();
                this.events.disable();
                this.navigationUI.disable();
                progFnc = this.slowNetworkGif.calc;
            }
            this.pageBuffer.loadMultiplePages(neededForLead, {
                done: function () {
    ...
    changeCurPage_dispatch(newPage, cbs) {
        ...
        if (typeof newPage === "undefined") {
            ...
            this.navigationUI.update(newPage, this.data.pages.length);
            // this.navigationUI.disable();
        ...
        if (curObj.vis) {
            ...
            if (text !== "" || curObj.editing) {
                ...
                for (let l = 0; l < lines.length; l++) {
                    ...
                    if (curObj.highlight && typeof curObj.highlight.wordNum === "number" &&
    ...
    draw_video(ctx, curVideo, curPage) {
        let elem = (curVideo.fSrc) ?
            this.pageBuffer.assetListLoaders[curPage].byFileSource[curVideo.fSrc].elem :
            this.pageBuffer.assetListLoaders[curPage].byFileName[curVideo.name].elem;
        let objDesc = this.getRealObjDescription(curVideo);
        //console.log(objDesc); This is weird....Jason, wat were you doing? 
        /*objDesc = {
            angle: 1,
            height: 64,
            layer: 19,
    ...
    set volume(bool) {
        this.runtimeProps.masterVolume = bool;

        // Reset volume 
        this.sequence.players.audios.map((e) => {
            e.elem.volume = this.runtimeProps.masterVolume;
        });
        this.sequence.players.videos.map((e) => {
            e.elem.volume = this.runtimeProps.masterVolume;
        });
        this.pageBuffer.assets.map((e) => {
            if (e.elem &&
                e.elem.volume) {
                e.elem.volume = this.runtimeProps.masterVolume;
            }
    ...
    setScale(to) {
        let scaleType = (to) ? to : this.runtimeProps.scaleType;
        let scaleInt;
        if (scaleType == "max" || scaleType == "fit") {
            let width = this.data.info.width;
            if (this.data.info.display == "composite") {
                width = width *= 2;
            }
            scaleInt = Math.min(
                window.innerWidth / (width + 5),
                window.innerHeight / (this.data.info.height + 5)
            );
            if (to == "max") {
                scaleInt = Math.max(1, scaleInt);
            }
        } else if (scaleType == "original") {
            scaleInt = 1;
        }
        this.runtimeProps.scaleType = scaleType;
        // scaleInt = 0.5;
        this.dom.main.parent().parent().css({
            "transform": `scale(${scaleInt})`,
        });
        this.currentScale = scaleInt;
    }
    ...
    updateNavigation(newPage) {
        newPage = (typeof newPage == "number") ? newPage : this.curPage;
        if (this.data.info.display == "composite" && (newPage == 0 || (newPage == this.data.pages.length - 1 && !this.data.info.lastPageSpread))) {
            this.navigationUI.coverWidth();
            this.onCoverPage = true;
        } else {
            this.navigationUI.spreadWidth();
            this.onCoverPage = false;
        }

        let elements = ["left", "right", "pulltab"];
        if (newPage == 0) {
            elements = ["right", "pulltab"];
        } else if (newPage == this.data.pages.length - 1) {
            elements = ["left", "pulltab"];
        }
        // enable also shows
        if (this.data.info.navigation) {
            this.navigationUI.enable(elements);
        } else {
            this.navigationUI.pulltab.show();
        }
    }
    ...
    constructor(data, runtimeProps) {
        ...
        this.runtimeProps = Object.assign(
            {
                ...
                // Market addition, a timed preview mode. Loose security.
                timeLimit: 0,
        ...
        // DEBUGS
        // this.runtimeProps.holdPageLinks = true;
        ...
        // TODO: improve, but at least something
        if (this.runtimeProps.timeLimit && parseInt(this.runtimeProps.timeLimit) > 0) {
            window.setTimeout(function () {
                history.back();
            }, this.runtimeProps.timeLimit * 1000);
        }

        let brandingAssets = (this.runtimeProps.brandingAssets) ? this.runtimeProps.brandingAssets : new BrandingAssets();
        ...
        // IOS needs a user interaction to play an audio file, but it's one interaction per player.
        // So we're using this hacky class to route all ios audio events in sequencing... TODO: video
        this.audioPlayer = new AudioPlayer(this.dom.audCont[0]);

        if (this.runtimeProps.isMobile) {
            this.forceOrientation = new ForceOrientation(document.body, 90);
        }

        // ALWAYS fit to window.
        this.setScale("fit");

        this.navigationUI = new Pubbly_UI(
            this.dom.uiCont[0],
            {
                animLength: 500,
                ...
                pulltabPosition: this.data.info.pulltabPosition || ["top", "right"],
                pulldownStyle: this.data.info.pulldownStyle || "full"
            },
            {
                left: () => {
                    pubbly.events.prev();
                ...
                enterFullscreen: function () {
                    ...
                    this.navigationUI.pulldown.hide();
                }.bind(this),
                exitFullscreen: function () {
                    // this.setScale("original");
        ...
        if (this.data.info.display == "composite" && this.curPage == 0 || (this.curPage == this.data.pages.length - 1 && !this.data.info.lastPageSpread)) {
            this.navigationUI.coverWidth();
            this.onCoverPage = true;
        } else {
            this.onCoverPage = false;
        ...
        this.progressGraph = new ProgressGraph_LoadLetters({
            cont: this.dom.main.find("#loaderCont")[0],
            imgSrc: brandingAssets.getSrc("logoLetters"),
            maxWidth: 600,
        });
        this.slowNetworkGif = new ProgressGraph_SpinningWheel({
            cont: this.dom.main.find("#loaderCont")[0],
            imgSrc: `${this.runtimeProps.rootToEngine}/assets/loader_carrot_eat.gif`
        });
        this.slowNetworkGif.hide();
