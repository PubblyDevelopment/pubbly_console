function Events(pubblyScope) { // Scopped contstants are _InitCapsCamelCase
    ...
    this.domOffsets = {
        x: $("#canvases").offset().left,
        y: $("#canvases").offset().top,
    };
    ...
    this.getDir = function (speedArr) {
        ...
        let consistantDir = false;
        let first = speedArr[0][0];
        let last = speedArr[speedArr.length - 1][0];
        if (Math.abs(first - last) > 4) {
            consistantDir = (first - last < 0) ? "left" : "right";
        } else {
    ...
    this.raw = {
        ...
        hover: function (e) {
            let loc = this.getLoc(e, this.offsets, this.domOffsets, _Pubbly.currentScale);
            // console.log(loc);
        ...
        drag: function (e) {
            ...
            if (this.m.action == "dragging" || this.m.action == "cloning") {
                ...
                if (this.m.turning) {
                    ...
                    let fullWidth = _pUnit;
                    if (_Pubbly.data.info.display == "composite") {
                        if (_Pubbly.onCoverPage) {
                            // fullWidth /= 2;
                        } else {
                            fullWidth *= 2;
                        }
                    ...
                    if (percent > 0.85) {
                        this.raw.up.call(_Pubbly.events, e);
                    } else {
        ...
        up: function (e) {
            if (this.m.down) {
                if (!_Pubbly.sequence.running) {
                    let loc = this.getLoc(e, this.offsets, this.domOffsets, _Pubbly.currentScale);
                    let log = [loc[0] / _Pubbly.data.info.width, loc[1] / _Pubbly.data.info.height];
                    // document.querySelector(".log").innerHTML += ` - ${e.changedTouches[0].pageX}`;
                    // document.querySelector(".log").innerHTML += ` - ${JSON.stringify(this.offsets)}`;

                    let caught = false;
                    if (this.m.action == "turning") {
                        // finish turn
                        if (this.getDir(this.m.lastMouseLocs) == this.m.turning ||
                            _Pubbly.turns.lastPercent > 0.85) {
                        ...
                        let then = now();
                        // Has to be at least >1/4 a second to register as an intentional click.
                        if (this.m.start && then - this.m.start[2] < 250) {
                            caught = _Pubbly.checkLocFor(loc, "clicks");
                        } else {
                            // console.info(`Too slow: ${this.m.down - then}ms`);
                        }
                    }
    ...
    function attach() {
        ...
        let outHandler = function (e) {
            e.preventDefault();
            if (_Events.enabled) {
                _Events.raw.out.call(_Events, e);
            }
        };
        ...
        ["mouseout"].forEach(function (e) {
            _Pubbly.dom.canvases[0].addEventListener("mouseout", outHandler, false);
        });
        $(window).resize(function () {
            _Pubbly.setScale();
            let o = $("#canvases").offset();
            _Pubbly.events.domOffsets = {
                x: o.left,
                y: o.top,
            }
        });
