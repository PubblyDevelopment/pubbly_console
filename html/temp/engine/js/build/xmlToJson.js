...
function updateJsonFromXML(xmlDoc, cbs) {
    ...
    let infoSet = [
        ...
        ["pulltabPosition", false, ["top", "right"]],
        ["pulldownStyle", false, "full"],
    ];
    ...
    /*
        The ONLY way we can guess is by splitting up the page name itself
        Default page namgin is num-num, so if the last page is just
            num
        Then it's _probably_ single, and if it's 
            num-num
        then it's _probably_ double.
    
        It would be nicer to get the last page name after the page info set loop
        But the page info set loop has object and link set loops
        and those require lastPageSpread in order to figure out some OTHER hacky BS

        So we have to get it HERE before it's used THERE.
        Hence the ugly and somewhat copied XML calls.
    */
    if (typeof xml.info.lastPageSpread === "undefined") {
        let pagesXML = xmlDoc.getElementsByTagName("Pages")[0].getElementsByTagName("Page");
        let lastPage = pagesXML[pagesXML.length - 1];
        let lastPageName = lastPage.getElementsByTagName("PageName")[0].innerHTML.split("-");
        if (lastPageName.length > 1 && (!isNaN(lastPageName[0] * 1) && !isNaN(lastPageName[1] * 1))) {
            xml.info.lastPageSpread = true;
        } else {
            // If I can't infer, I guess single.
    ...
    if (xml.info.display === "composite" && pageNodes.length <= 2) {
        ...
        for (let p = 0; p < pageNodes.length; p++) {
                ...
                if (objSets[type]) {
                    ...
                    for (let os = 0; (os < curSet.length && curSet[os]); os++) {
                        try {
                            ...
                            if (curObj.name) {
                                console.error(`Error getting ${curSet[os][1]} from ${curObj.name}`)
                            }
                            if (e.message) {
                            ...
                            if (e.fatal) {
                                error(true, "load", "");
                                console.log(" -- Breaking JS execution (Ignore TypeError below) -- ");
                                // NOT a real function, force stop script exec
                                cbs.fail();
                                e.break_exec();
                            }
                        }
            ...
            for (let l = 0; l < linkNodes.length; l++) {
                ...
                /*
                Oh no wait its much more complicated

                (for sake of clarity, all links are at 0,0 point, relative to page)

                FRONT covers ALWAYS have stupid offsets
                BACK covers last page SINGLE have stupid offsets
                BACK covers last page DOUBlE DO NOT have offsets
                */
                if (xml.info.display == "composite" &&
                    (
                        // FRONT covers ALWAYS have offsets
                        p == 0 ||
                        // BACK covers ONLY have offsets on last page SINGLE
                        (p == pageNodes.length - 1 && !xml.info.lastPageSpread)
                    )
                ) {
                    // Polys are given to me as [left,top].
                    // And the point in poly we snatched expects [x,y] [left,top] format.
                    // so... TODO: Fix this mess
                    if (p == pageNodes.length - 1 && !xml.info.lastPageSpread) {
                        console.log(this);
                    }
    ...
    for (let p = 0; p < xml.pages.length; p++) {
        ...
        for (let o = 0; o < curPage.objs.length; o++) {
            ...
            curObj.init.angle = curObj.angle;
            if (curObj.mobility === "drag") {
