...
function Build(props, cbs) {
    ...
    this.props = Object.assign({
        ...
        xmlStr: false,
        rootToEngine: "",
        // For a faster debug, skips over the api POST of the converted XML data, and just runs it right into the new Pubbly call.
        skipBuild: false,
        environment: "online",
        // relPath to library of common assets, shared between books
        // Note, sharedLibrary does not need to have _every_ book asset. Just _some_
        sharedLibrary: false,
    ...
    let preflight = new Preflight({
        ...
        skipBuild: this.props.skipBuild,
        brandingAssets: this.props.brandingAssets
    });
    function xmlRecieved(xmlDoc) {
        updateJsonFromXML(xmlDoc, {
            fail: function () {
                preflight.progressGraph.say("ERROR: XML did not parse. Check web console for more details.");
                preflight.progressGraph.error();
            },
                ...
                json.pages.map(page => {
                    page.objs.map(obj => {
                        if (obj.type == "image") {
                            ...
                            obj.relPath = _Build.props.bookLoc + "/images/" + obj.fileName + ".mp4";
    ...
    if (this.props.xmlStr) {
        ...
        $.get(_Build.props.bookLoc + "/" + this.props.xmlName + "?" + rand(0, 100000)).success(function (xmlDoc) {
            ...
            preflight.progressGraph.say("ERROR: XML not found. Please double check that a zip file has been uploaded via the admin tools");
            preflight.progressGraph.fail();
        });
    }
}
