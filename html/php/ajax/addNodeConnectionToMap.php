...
if (LOGGED_IN) {
    ...
    $info = $query->fetchArray("SELECT CONCAT('map/', mp.name, '/', mn.name, '/MainXML.xml') AS xmlPath, fmnp.from_link_name AS linkName, fmnp.from_link_page AS linkPage FROM map_node_path fmnp RIGHT JOIN map_node mn ON fmnp.from_node_id = mn.map_node_id RIGHT JOIN map mp ON mp.map_id = mn.map_id WHERE fmnp.map_node_path_id = ?", ["s", $fromPathID]);


    $linkName = $info['linkName'];
    $page = $info['linkPage'];
    $url = "t=m&mid=$mapID&nid=$toNodeID";
